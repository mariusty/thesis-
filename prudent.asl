	...
	?risk(Risk); //get risk
    	if (R >= P) { //if reputation is higher than price
    	 	if (P <= R*0.5 ) { //price is between 0.5R and R
				...
				if (P + Risk < 100 ) { //if price and risk sum is under 100
					S = P + Risk; //set sureness equal to price+risk
				}
				else{
					S = 100; //set sureness equal to 100
				}
			...
			if (P >= R*1.5 ) { //price is over 1.5R
				...
				if (P - Risk < 100 ) {  //if price - risk is under 100
					S = P - Risk; //set sureness equal to price-risk
				}
				else{
					S = 100; //set sureness equal to 100
...
+good_result(Newreput)[source(A)] //agent receives message that initiator is satisfied
	...
	if (Risk <95 ) { //increase risk for 5 points if it is possible
		K = Risk + 5;
		-+risk(K);
	}. 
