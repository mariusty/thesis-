...
result (Sur, Rep, B) :- .random(R)& B = math.round(Rep*0.6+Sur*0.2+(100*R)*0.2). //calculation of the predicted result
...
-+risk(0); //set the default risk tolerance to 0
    	?random (R1);
    	?random (R2);
    	?random (R3);
    	?random (R4);
...
+request(Task, P) [source(A)] //initiator sent a request with task price
	...
	?risk(Risk); //get risk
    	if (R >= P) { //if reputation is higher than price
			if (P <= R*0.5 ) { // price is under 0.5R
				S = 0; //agent doesn't want to participate
			}
			else { // price is between 0.5R and R
				S = 100; //set sureness equal to 100
			}
		}
		else { //if price is higher than reputation
			if (P >= R*1.5 ) { //price is over 1.5R
				if (P-R+Risk < 100 ) { //if price-reputation+risk is under 100					
					S = P-R+Risk; //set sureness equal to price-reputation+risk
				}
				else { 
					S = 100; //set sureness equal to 100						
				}			
...
+reject (Ns) //initiator sent message that agent hasn't been chosen
    <- ?risk(Risk); //get current risk
    if (Risk <91 ) { //increase risk for 10points if possible
		K = Risk + 10;
		-+risk(K);
		.print("I haven't been chosen for ", Ns, " I'll up my risks to ", K);
	}
	else{
		.print("I haven't been chosen for ", Ns, " my risk is ", Risk);
	}.
...
+bad_result(Newreput)[source(A)] //agent receives message that initiator is not satisfied
	...
	?risk(Risk); //get current risk tolerance
	if (Risk > 5 ) { //decrease risk for 5 points if possible
		K = Risk - 5; 
		-+risk(K); //set new risk value
	}
	?reputation (Rep);
	?risk (Ris);
